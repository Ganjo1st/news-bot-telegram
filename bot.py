"""
🤖 Telegram News Bot - Версия 21.0
ДИАГНОСТИЧЕСКАЯ ВЕРСИЯ ДЛЯ 9111.RU
- Подробное логирование всех запросов
- Сохранение ответов сервера
- Тестирование каждого шага отдельно
"""
...
class NewsBot:
    ...
    def _find_chrome(self) -> str:
        ...
        logger.info("ℹ️ Chrome не найден, но для 9111.ru он не требуется")
        return None
    ...
    # ========== ДИАГНОСТИЧЕСКАЯ ФУНКЦИЯ ДЛЯ 9111.RU ==========
    def diagnose_9111(self):
        """
        Полная диагностика подключения к 9111.ru
        Сохраняет все ответы сервера для анализа
        """
        logger.info("=" * 80)
        logger.info("🔍 ДИАГНОСТИКА 9111.RU")
        logger.info("=" * 80)
        ...
        logger.info("1️⃣ Проверка доступности сайта...")
        try:
            response = requests.get('https://www.9111.ru', timeout=10)
            logger.info(f"   Статус: {response.status_code}")
            logger.info(f"   Заголовок: {BeautifulSoup(response.text, 'html.parser').title.string if BeautifulSoup(response.text, 'html.parser').title else 'Нет заголовка'}")
            results['site_accessible'] = response.status_code == 200
            with open(f"diagnostic_9111_main_{timestamp}.html", 'w', encoding='utf-8') as f:
                f.write(response.text)
            logger.info(f"   ✅ HTML сохранён: diagnostic_9111_main_{timestamp}.html")
        except Exception as e:
            logger.error(f"   ❌ Ошибка: {e}")
            results['site_accessible'] = False
        
        # ШАГ 2: Проверка страницы авторизации
        logger.info("2️⃣ Проверка страницы авторизации...")
        try:
            session = requests.Session()
            session.headers.update({'User-Agent': 'Mozilla/5.0'})
            
            # Получаем главную для cookies
            session.get('https://www.9111.ru', timeout=10)
            
            # Пробуем отправить POST на авторизацию
            login_data = {
                'email': NINTH_EMAIL,
                'pass': NINTH_PASSWORD,
                'action': 'login',
            ...
            login_response = session.post(
                'https://www.9111.ru/ajax/auth.php',
                data=login_data,
                timeout=10,
            ...
            logger.info(f"   URL после редиректа: {login_response.url}")
            logger.info(f"   Размер ответа: {len(login_response.text)} байт")
            logger.info(f"   Первые 200 символов ответа: {login_response.text[:200]}")
            
            with open(f"diagnostic_9111_login_{timestamp}.html", 'w', encoding='utf-8') as f:
                f.write(login_response.text)
            logger.info(f"   ✅ HTML сохранён: diagnostic_9111_login_{timestamp}.html")
            
            if 'success' in login_response.text.lower() or 'ok' in login_response.text.lower():
        ...
        # ШАГ 3: Проверка страницы создания публикации (без авторизации)
        logger.info("3️⃣ Проверка страницы создания публикации...")
        try:
            response = requests.get('https://www.9111.ru/pubs/add/title/', timeout=10)
            logger.info(f"   Статус: {response.status_code}")
            logger.info(f"   URL: {response.url}")
            
            soup = BeautifulSoup(response.text, 'html.parser')
            if soup.find('form', {'id': 'form_create_topic_group'}):
                logger.info("   ✅ Форма создания найдена")
            else:
                logger.warning("   ⚠️ Форма создания не найдена")
                
            with open(f"diagnostic_9111_addpage_{timestamp}.html", 'w', encoding='utf-8') as f:
                f.write(response.text)
            logger.info(f"   ✅ HTML сохранён: diagnostic_9111_addpage_{timestamp}.html")
            
        except Exception as e:
            logger.error(f"   ❌ Ошибка: {e}")
        
        # ШАГ 4: Проверка CSRF токена
        logger.info("4️⃣ Поиск CSRF токена...")
        try:
            response = requests.get('https://www.9111.ru/pubs/add/title/', timeout=10)
            soup = BeautifulSoup(response.text, 'html.parser')
            ...
            if csrf_input:
                ...
                logger.info(f"   ✅ CSRF токен найден: {csrf_token[:20]}...")
                results['csrf_found'] = True
            else:
                logger.warning("   ⚠️ CSRF токен не найден")
                results['csrf_found'] = False
        except Exception as e:
            logger.error(f"   ❌ Ошибка: {e}")
            results['csrf_found'] = False
        
        # ШАГ 5: Проверка рубрик
        logger.info("5️⃣ Проверка рубрик...")
        try:
            response = requests.get('https://www.9111.ru/pubs/add/title/', timeout=10)
            soup = BeautifulSoup(response.text, 'html.parser')
            rubric_select = soup.find('select', {'id': 'rubric_id2'})
            if rubric_select:
                options = rubric_select.find_all('option')
                logger.info(f"   ✅ Найдено рубрик: {len(options)}")
                news_found = False
                for opt in options:
                    if 'Новости' in opt.text:
                        logger.info(f"   ✅ Рубрика 'Новости' найдена, value={opt.get('value')}")
                        news_found = True
                        break
                if not news_found:
                    logger.warning("   ⚠️ Рубрика 'Новости' не найдена")
            else:
                logger.warning("   ⚠️ Выпадающий список рубрик не найден")
        except Exception as e:
            logger.error(f"   ❌ Ошибка: {e}")
    ...
    def publish_to_9111(self, title, content, source_url):
        """
        Публикация на 9111.ru через requests с подробным логированием
        """
        ...
        try:
            ...
            logger.info(f"   URL после редиректа: {login_response.url}")
            logger.info(f"   Размер ответа: {len(login_response.text)} байт")
            logger.info(f"   Первые 100 символов: {login_response.text[:100]}")
            
            # Сохраняем ответ для анализа
            with open(f"debug_9111_login_{timestamp}.html", 'w', encoding='utf-8') as f:
                f.write(login_response.text)
            logger.info(f"   ✅ Ответ сохранён: debug_9111_login_{timestamp}.html")
            
            # Проверяем успешность
            if 'success' not in login_response.text.lower() and 'ok' not in login_response.text.lower():
                logger.error(f"❌ Ошибка авторизации")
                return False
            
            logger.info("✅ Авторизация успешна")
            
            # ШАГ 3: Получаем страницу создания публикации
            ...
            # Сохраняем страницу
            with open(f"debug_9111_addpage_{timestamp}.html", 'w', encoding='utf-8') as f:
                f.write(add_page.text)
            logger.info(f"   ✅ Страница сохранена: debug_9111_addpage_{timestamp}.html")
            ...
            if csrf_input:
                csrf_token = csrf_input.get('value')
                logger.info(f"   ✅ Найден CSRF токен: {csrf_token[:20]}...")
            
            # Находим рубрику Новости
            rubric_id = '382235'  # значение по умолчанию
            rubric_select = soup.find('select', {'id': 'rubric_id2'})
            if rubric_select:
                for option in rubric_select.find_all('option'):
                    if 'Новости' in option.text:
                        rubric_id = option.get('value')
                        logger.info(f"   ✅ Найдена рубрика 'Новости' с ID: {rubric_id}")
                        break
            ...
            logger.info(f"   Данные формы: {list(post_data.keys())}")
            
            publish_response = session.post(
                'https://www.9111.ru/pubs/add/title/',
                data=post_data,
                allow_redirects=True,
                timeout=15
            )
            ...
            logger.info(f"   Размер ответа: {len(publish_response.text)} байт")
            
            # Сохраняем ответ
            with open(f"debug_9111_result_{timestamp}.html", 'w', encoding='utf-8') as f:
                f.write(publish_response.text)
            logger.info(f"   ✅ Результат сохранён: debug_9111_result_{timestamp}.html")
            ...
            if publish_response.status_code == 200:
                if 'Спасибо' in publish_response.text or 'опубликована' in publish_response.text:
                    ...
                    # Ищем ключевые слова
                    if 'ошибка' in publish_response.text.lower():
                        logger.error("   В ответе есть слово 'ошибка'")
                    if 'успешно' in publish_response.text.lower():
                        logger.info("   В ответе есть слово 'успешно'")
                    return True
            else:
    ...
    async def process_and_publish(self):
        ...
        # ЗАПУСКАЕМ ДИАГНОСТИКУ 9111.ru (один раз)
        if not hasattr(self, 'diagnostic_run'):
            logger.info("🔧 Запуск диагностики 9111.ru...")
            loop = asyncio.get_event_loop()
            await loop.run_in_executor(None, self.diagnose_9111)
            self.diagnostic_run = True
    ...
    async def start(self):
        logger.info("=" * 80)
        logger.info("🚀 NEWS BOT 21.0 - ДИАГНОСТИЧЕСКАЯ ВЕРСИЯ")
        logger.info("=" * 80)
